...
const pickingType = computed(() => {
  if (route.path.includes('incoming')) return 'incoming'
  if (route.path.includes('outgoing')) return 'outgoing'
  return 'internal'
})

const title = computed(() => {
  const map = { incoming: '入库调拨', outgoing: '出库调拨', internal: '内部调拨' }
  return map[pickingType.value] || '调拨单'
})
