...
router = APIRouter(prefix="/lots", tags=["批次管理"])


@router.get("/{lot_id}", response_model=StockLotResponse, summary="获取批次详情")
...
@router.post("", response_model=StockLotResponse, summary="创建批次")
...
@router.put("/{lot_id}", response_model=StockLotResponse, summary="更新批次")
...
@router.delete("/{lot_id}", response_model=MessageResponse, summary="删除批次")
...
@router.get("", response_model=ListResponse, summary="获取批次列表")
async def list_lots(
