...
@location_router.get("/{location_id}", response_model=StockLocationResponse, summary="获取库位详情")
...
@location_router.post("", response_model=StockLocationResponse, summary="创建库位")
...
@location_router.put("/{location_id}", response_model=StockLocationResponse, summary="更新库位")
...
@location_router.delete("/{location_id}", response_model=MessageResponse, summary="删除库位")
async def delete_location(location_id: int):
    ...
    try:
        ...
        if not success:
            raise HTTPException(status_code=404, detail="库位不存在")
        return {"message": "库位删除成功", "success": True}
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))


@location_router.get("", response_model=ListResponse, summary="获取库位列表")
