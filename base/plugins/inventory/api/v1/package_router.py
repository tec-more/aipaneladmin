...
router = APIRouter(prefix="/packages", tags=["包裹管理"])


@router.get("/{package_id}", response_model=StockPackageResponse, summary="获取包裹详情")
...
@router.post("", response_model=StockPackageResponse, summary="创建包裹")
...
@router.put("/{package_id}", response_model=StockPackageResponse, summary="更新包裹")
...
@router.delete("/{package_id}", response_model=MessageResponse, summary="删除包裹")
async def delete_package(package_id: int):
    ...
    try:
        ...
        if not success:
            raise HTTPException(status_code=404, detail="包裹不存在")
        return {"message": "包裹删除成功", "success": True}
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))


@router.get("", response_model=ListResponse, summary="获取包裹列表")
