...
router = APIRouter(prefix="/quants", tags=["库存查询"])


@router.get("/{quant_id}", response_model=StockQuantResponse, summary="获取库存详情")
...
@router.get("", response_model=ListResponse, summary="获取库存列表")
...
@router.get("/summary", response_model=StockQuantSummary, summary="库存汇总统计")
...
@router.get("/by-product/{product_code}", summary="按产品查询库存")
...
@router.get("/by-location/{location_id}", summary="按库位查询库存")
